...
#...!...!....................
def rotation_block(circ,nAnc, ir, addRz=True):
    # assumes nb of params scales with  num of qubits
    nq=circ.num_qubits-nAnc
    nPar=2*nq-1 if addRz else nq
    thetas = ParameterVector('th%d'%ir, length=nPar)
    for i in range(nq): circ.ry(thetas[i],i+nAnc)
    if addRz:
        for i in range(nq-1):  circ.rz(thetas[i+nq],i)

#...!...!....................
def entangling_block(qc,nAnc,cxDir=0,doCircle=False):
    nq=qc.num_qubits-nAnc
    for i in range(nq-1):
        ictr=i+nAnc; itgt=ictr+1
...
#...!...!....................
def bias_block(qc,ir,nAnc):
    nq=qc.num_qubits
    assert nAnc==1
    phi = Parameter('ph%d'%ir)
    iq=0
    it=ir+nAnc
    qc.reset(iq)
    qc.rx(phi,iq)
...
#...!...!....................
def myAnsatz(nFeat,nReps, meas_list,nAnc=0,barrier=True):
    nBits=len(meas_list)
    assert nBits>0
    qc = qk.QuantumCircuit(nFeat+nAnc,nBits,name='ansatz_r%d'%nReps)
    for ir in range(nReps):
        rotation_block(qc,nAnc,ir, addRz=False)
        if barrier: qc.barrier()
        entangling_block(qc,nAnc,cxDir=0,doCircle=False)
        #if ir==0: bias_block(qc,ir,nAnc)
        if barrier: qc.barrier()
    add_meas_block(qc,nAnc,ir=nReps)
    return qc
...
def build_circuit(X,nReps,verb=1):
    ...
    if verb>0: print('\nFeatures'); print(qc1)
    measL=[2,3]
    qc2=myAnsatz(nFeat,nReps, measL,nAnc)
    if verb>0: print('\nAnsatz'); print(circuit_drawer(qc2, output='text',cregbundle=False))

    #print('nBits=',nBits)
    nqTot=nFeat+nAnc
    circuit = qk.QuantumCircuit(nqTot,nBits)
    circuit.append(qc1, range(nAnc,nqTot))
    circuit.append(qc2, range(nqTot),range(nBits)) 
